...
func NewTxBuilder(
	...
	simulateAndExecute bool, chainID, memo string, fees Coins, gasPrices DecCoins) TxBuilder {
	return TxBuilder{
		txEncoder:          txEncoder,
		keybase:            nil,
...
func NewTxBuilderFromCLI(input io.Reader) TxBuilder {
	kb, err := crkeys.NewKeyring(sdk.KeyringServiceName(), viper.GetString(flags.FlagKeyringBackend), viper.GetString(flags.FlagHome), input)
	if err != nil {
		panic(err)
	}
	txbldr := TxBuilder{
	...
	txbldr = txbldr.WithPayer(viper.GetString(FlagPayer))
...
func (bldr TxBuilder) FeePayer() types.AccountID {
	...
	res, err := types.NewAccountIDFromStr(bldr.payer)
	if err != nil {
		panic(err)
	}
	return res
...
func (bldr TxBuilder) WithGasPrices(gasPrices string) TxBuilder {
	...
	if parsedGasPrices.IsZero() {
		parsedGasPrices = constants.MinGasPrice
	}
...
func (bldr TxBuilder) BuildSignMsg(msgs []sdk.Msg) (StdSignMsg, error) {
	...
	return StdSignMsg{
		...
		Memo:          bldr.memo,
		Msg:           msgs,
		Fee:           NewStdFee(bldr.gas, bldr.FeePayer(), fees),
	}, nil
}
...
func (bldr TxBuilder) Sign(name, passphrase string, msg StdSignMsg) ([]byte, error) {
	...
	return bldr.txEncoder(NewStdTx(msg.Msg, msg.Fee, []StdSignature{sig}, msg.Memo))
}
...
func (bldr TxBuilder) BuildTxForSim(msgs []sdk.Msg) ([]byte, error) {
	...
	sigs := []StdSignature{{}}
	return bldr.txEncoder(NewStdTx(signMsg.Msg, signMsg.Fee, sigs, signMsg.Memo))
}
