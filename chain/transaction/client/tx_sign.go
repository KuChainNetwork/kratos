...
func makeSignCmd(cdc *codec.Codec) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		if multisigAddrStr != "" {
			...
			newTx, err = txutil.SignStdTxWithSignerAddress(
				txBldr, cliCtx, multisigAddr, cliCtx.GetFromName(), stdTx, offline,
			)
			generateSignatureOnly = true
		} else {
			appendSig := viper.GetBool(flagAppend) && !generateSignatureOnly
			newTx, err = txutil.SignStdTx(
				txBldr,
				cliCtx,
				cliCtx.GetFromName(),
				stdTx, appendSig, offline)
		}
...
func printAndValidateSigs(
	...
	for i, sig := range sigs {
		...
		if !offline && success {
			id := cliCtx.GetAccountID()
			num, seq, err := txutil.NewAccountRetriever(cliCtx).GetAuthNumberSequence(id)
			if err != nil {
