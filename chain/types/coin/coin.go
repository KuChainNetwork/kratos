...
var (
	// Denominations can be 3 ~ 17 characters long.
	reCreatorName  = `[a-z][a-z0-9]{2,16}`
	reSymbolString = `[a-z][a-z0-9]{2,16}`
	reDnmString    = fmt.Sprintf(`%s\/%s`, reCreatorName, reSymbolString)
	reAmt          = `[[:digit:]]+`
