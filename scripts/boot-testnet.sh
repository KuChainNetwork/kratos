...
printf "start testnet for chain ${CHAIN_ID} in ${ROOT_DIR}\\n"

PARAMS="--home ${ROOT_DIR}/node/"
PARAMSCLI="--home ${ROOT_DIR}/cli/ --keyring-backend test"

./build/kucli ${PARAMSCLI} keys add chain

VALKEY=$(./build/kucli ${PARAMSCLI} keys show chain -a)
...
./build/kucd ${PARAMS} genesis add-account chain ${VALKEY}
...
./build/kucd ${PARAMS} genesis add-coin "1000000000000000000000000000000000000000chain/sys" "main token"
#./build/kucd ${PARAMS} genesis add-coin "1000000000000000000000000000000000000000validatortoken" "for staking"
./build/kucd ${PARAMS} genesis add-account-coin ${VALKEY} "100000000000000000000000chain/sys"
./build/kucd ${PARAMS} genesis add-account-coin chain "100000000000000000000000chain/sys"

printf "./build/kucd ${PARAMS} gentx ${VALKEY} --keyring-backend test --name chain --home-client ${ROOT_DIR}/cli/\\n"
./build/kucd ${PARAMS} gentx ${VALKEY} --keyring-backend test --name chain --home-client ${ROOT_DIR}/cli/
