...
func createCoinExt(t *testing.T, app *simapp.SimApp, isSuccess bool,
	creator types.AccountID, symbol types.Name, maxSupply types.Coin, canIssue, canLock bool, issue2Height int64, initSupply types.Coin, desc []byte) error {
	ctx := app.NewTestContext()
	creatorName := creator.MustName()
...
func TestCreateAsset(t *testing.T) {
	...
	Convey("test create asset desc too large", t, func() {
		...
		So(createCoinExt(t, app, false, account4, symbol, maxSupply, true, true, 0, initSupply, desc),
	...
	Convey("test create symbol error asset", t, func() {
		...
		So(createCoinExt(t, app, false, account4, symbol, maxSupplyErr1, true, true, 0, initSupply, desc),
			simapp.ShouldErrIs, assetTypes.ErrAssetSymbolError)

		So(createCoinExt(t, app, false, account4, symbol, maxSupplyErr2, true, true, 0, initSupply, desc),
			simapp.ShouldErrIs, assetTypes.ErrAssetSymbolError)

		So(createCoinExt(t, app, false, account4, symbol, maxSupply, true, true, 0, initSupplyErr, desc),
			simapp.ShouldErrIs, assetTypes.ErrAssetSymbolError)
	})
}
...
func TestLockCoinsError(t *testing.T) {
	...
	Convey("test lock err by coins type cannot be locked", t, func() {
		...
		So(createCoinExt(t, app, true, account2, symbol, maxSupply, true, false, 0, initSupply, desc), ShouldBeNil)
		// no issue, so account2 no have this coin
