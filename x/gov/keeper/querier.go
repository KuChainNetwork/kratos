...
func NewQuerier(keeper Keeper) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) ([]byte, error) {
		...
		case types.QueryPunishValidators:
			return queryPunishedValidators(ctx, path[1:], req, keeper)

		case types.QueryPunishValidator:
			return queryPunishedValidator(ctx, path[1:], req, keeper)
...
func queryTally(ctx sdk.Context, path []string, req abci.RequestQuery, keeper Keeper) ([]byte, error) {
	...
	default:
		...
		_, _, tallyResult, _, _, _ = keeper.Tally(ctx, proposal)
	}
...
func queryPunishedValidators(ctx sdk.Context, _ []string, req abci.RequestQuery, keeper Keeper) ([]byte, error) {
	validators := keeper.GetPunishValidators(ctx)
	bz, err := codec.MarshalJSONIndent(types.ModuleCdc, validators)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrJSONMarshal, err.Error())
	}
	return bz, nil
...
func queryPunishedValidator(ctx sdk.Context, _ []string, req abci.RequestQuery, keeper Keeper) ([]byte, error) {
	var params types.QueryPunishValidatorParams
	err := keeper.cdc.UnmarshalJSON(req.Data, &params)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrJSONUnmarshal, err.Error())
	}

	validator, found := keeper.GetPunishValidator(ctx, params.ValidatorAccount)
	if !found {
		return nil, sdkerrors.Wrap(types.ErrValidatorNoPunish, err.Error())
	}

	bz, err := codec.MarshalJSONIndent(types.ModuleCdc, validator)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrJSONMarshal, err.Error())
	}
	return bz, nil
