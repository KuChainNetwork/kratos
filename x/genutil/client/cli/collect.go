...
func CollectGenTxsCmd(ctx *server.Context, cdc *codec.LegacyAmino, genBalIterator types.GenesisBalancesIterator,
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			genDoc, err := chainTypes.GenesisDocFromFile(config.GenesisFile())
			if err != nil {
			...
			appMessage, err := genutil.GenAppStateFromConfig(cdc, config, initCfg, *genDoc, genBalIterator, manager)
			if err != nil {
