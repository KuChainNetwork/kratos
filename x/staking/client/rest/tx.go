...
func postDelegationsHandlerFn(cliCtx txutil.KuCLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			chainTypes.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("amount parse error, %v", err))
			return
		}

		delAccountID, err := chainTypes.NewAccountIDFromStr(req.DelegatorAcc)
		if err != nil {
			chainTypes.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("delegate accountID error, %v", err))
			return
		}
		valAccountID, err := chainTypes.NewAccountIDFromStr(req.ValidatorAcc)
		if err != nil {
			chainTypes.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("validator accountID error, %v", err))
			return
		}

		delAccAddress, err := txutil.QueryAccountAuth(cliCtx, delAccountID)
		if err != nil {
			chainTypes.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("query account %s auth error, %v", delAccountID, err))
			return
		}
...
func postRedelegationsHandlerFn(cliCtx txutil.KuCLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		delAccountID, err := chainTypes.NewAccountIDFromStr(req.DelegatorAcc)
		if err != nil {
			chainTypes.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("delegate acccount error, %v", err))
			return
		}

		valSrcAccID, err := chainTypes.NewAccountIDFromStr(req.ValidatorSrcAcc)
		if err != nil {
			chainTypes.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("src-validator error, %v", err))
			return
		}

		valDstAccID, err := chainTypes.NewAccountIDFromStr(req.ValidatorDstAcc)
		if err != nil {
			chainTypes.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("dst-validator error, %v", err))
			return
		}
...
func postUnbondingDelegationsHandlerFn(cliCtx txutil.KuCLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		delAccountID, err := chainTypes.NewAccountIDFromStr(req.DelegatorAcc)
		if err != nil {
			chainTypes.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("delegate account id error, %v", err))
			return
		}

		amount, err := chainTypes.ParseCoin(req.Amount)
		if err != nil {
			chainTypes.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("parse amount error, %v", err))
			return
		}

		valAddr, err := chainTypes.NewAccountIDFromStr(req.ValidatorAcc)
		if err != nil {
			chainTypes.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("val account id error, %v", err))
			return
		}
		delAccAddress, err := txutil.QueryAccountAuth(cliCtx, delAccountID)
		if err != nil {
