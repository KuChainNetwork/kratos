...
func queryPool(ctx sdk.Context, k Keeper) ([]byte, error) {
	...
	pool := types.NewPool(
		k.bankKeeper.GetCoinPowerByDenomd(ctx, notBondedPool.GetID(), bondDenom).Amount,
		k.bankKeeper.GetCoinPowerByDenomd(ctx, bondedPool.GetID(), bondDenom).Amount,
	)
...
func delegationToDelegationResponse(ctx sdk.Context, k Keeper, del types.Delegation) (types.DelegationResponse, error) {
	val, found := k.GetValidator(ctx, del.ValidatorAccount)
	if !found {
...
func redelegationsToRedelegationResponses(
	...
	for i, redel := range redels {
		...
		resp[i] = types.NewRedelegationResponse(
			redel.DelegatorAccount,
			redel.ValidatorSrcAccount,
			redel.ValidatorDstAccount,
			entryResponses,
		)
	}
