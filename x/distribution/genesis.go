...
func InitGenesis(ctx sdk.Context, bk types.BankKeeperAccountID, supplyKeeper types.SupplyKeeperAccountID, keeper Keeper, data types.GenesisState) {
	...
	if bk.GetAllBalances(ctx, moduleAcc.GetID()).IsZero() && bk.GetCoinPowers(ctx, moduleAcc.GetID()).IsZero() {
		ctx.Logger().Info("genesis module account asset", "account", moduleAcc.GetID(), "asset", moduleHoldingsInt)
...
func ExportGenesis(ctx sdk.Context, keeper Keeper) types.GenesisState {
	...
	keeper.IterateValidatorOutstandingRewards(ctx,
		func(addr AccountID, rewards types.ValidatorOutstandingRewards) (stop bool) {
	...
	keeper.IterateValidatorAccumulatedCommissions(ctx,
		func(addr AccountID, commission types.ValidatorAccumulatedCommission) (stop bool) {
	...
	keeper.IterateValidatorHistoricalRewards(ctx,
		func(val AccountID, period uint64, rewards types.ValidatorHistoricalRewards) (stop bool) {
	...
	keeper.IterateValidatorCurrentRewards(ctx,
		func(val AccountID, rewards types.ValidatorCurrentRewards) (stop bool) {
	...
	keeper.IterateDelegatorStartingInfos(ctx,
		func(val AccountID, del AccountID, info types.DelegatorStartingInfo) (stop bool) {
	...
	keeper.IterateValidatorSlashEvents(ctx,
		func(val AccountID, height uint64, event types.ValidatorSlashEvent) (stop bool) {
