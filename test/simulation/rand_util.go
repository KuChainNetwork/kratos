...
// RandPositiveInt get a rand positive types.Int
func RandPositiveInt(r *rand.Rand, max types.Int) (types.Int, error) {
...
func RandomAmount(r *rand.Rand, max types.Int) types.Int {
...
func RandomDecAmount(r *rand.Rand, max types.Dec) types.Dec {
	var randInt = big.NewInt(0)
	switch r.Intn(10) {
	case 0:
		// randInt = big.NewInt(0)
...
// i.e. 0 amt in coins
func RandSubsetCoins(r *rand.Rand, coins types.Coins) types.Coins {
	if len(coins) == 0 {
		return types.Coins{}
	}
	...
	for i, c := range coins {
		...
		subset = append(subset, types.NewCoin(c.Denom, amt))
	}
	return subset.Sort()
}
